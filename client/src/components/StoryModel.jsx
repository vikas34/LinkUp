...
const StoryModel = ({ setShowModal, fetchStories }) => {
    ...
    if (file) {
      if (file.type.startsWith("video")) {
        if (file.size > MAX_VIDEO_SIZE_MB * 1024 * 1024) {
          toast.error(`Video file size can not exceed ${MAX_VIDEO_SIZE_MB}MB.`);
          setMedia(null);
          setPreviewUrl(null);
          return;
        }
  ...
  const handleCreateStory = async () => {
    ...
    if (media_type === "text" && !text) {
      throw new Error("Plz enter new text");
    }
  ...
  return (
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
      <div className="w-full  max-w-md">
        <div className="text-center flex items-center mb-4 justify-between ">
          <button
        ...
        <div className="flex gap-2 mt-4 ">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 cursor-pointer p-2 rounded ${
        ...
        to-purple-600 hover:to-purple-700 active:scale-95 transition cursor-pointer"
        >
